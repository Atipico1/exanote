...
**설정 ▸ AI 도구 연결**에서 Codex나 Claude Code를 연결하면, AI에게 지난 회의에서 결정한 내용이나 특정 발언을 찾아달라고 할 수 있습니다. 회의 목록과 요약, 화자별 전사를 검색하고 읽을 수 있지만 기록을 고치거나 지울 수는 없습니다. AI 도구에 보여 준 회의 내용은 해당 서비스로 전송됩니다.

## 내 컴퓨터에서 처리합니다

음성 인식, 화자 구분, 번역, 회의 노트 작성은 Mac에서 실행됩니다. Google Drive 공유와 AI 도구 연결은 직접 켰을 때만 사용합니다. 모델은 **설정 ▸ AI 모델**에서 설치하거나 지울 수 있어요.

## 성능 측정

동일한 한국어 토론 녹음에서 추출한 10개 구간(총 48분 23초)을 비교했습니다. Exanote는 Apple M4 · 메모리 32GB Mac에서, 아래 세 외부 모델은 OpenRouter API로 측정했습니다.
...
CER(Character Error Rate)은 **정답 전사와 비교해 문자 단위로 얼마나 정확하게 전사했는지 확인하는 지표**입니다. 잘못 바뀌거나 빠지거나 추가된 문자 수를 정답의 문자 수로 나눈 오류율로, **낮을수록 정확합니다.** 공백과 문장부호는 제외해 계산했습니다.

한 토론의 일부 구간에 대한 결과이며, Exanote 개선에도 사용한 데이터입니다. 일반적인 한국어 전사 성능 순위를 의미하지 않습니다. [측정 조건과 재현 방법](docs/benchmarks/transcription-2026-09-26.md)을 참고하세요.
