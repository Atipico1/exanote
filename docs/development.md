...
일반 설정의 글자 섹션에서 설치된 글꼴과 1–5단계 글자 크기 슬라이더를 조절합니다. 기본값은 시스템 글꼴과 가운데 3단계입니다. 하나의 단계가 본문과 사이드바를 함께 변경하며, 변경사항은 즉시 반영되고 앱을 다시 실행해도 유지됩니다. 기존 개별 pt 설정은 사용하지 않습니다. 회의 시작 알림 사용 여부와 macOS 알림 권한은 권한 화면에서 함께 관리합니다.

사이드바 행 높이와 섹션 여백도 선택한 단계에 비례합니다. macOS List 기본 글꼴과 행 높이가 사용자 설정을 덮어쓰지 않도록 각 행에 직접 적용합니다. 설정 행은 공간이 부족하면 설명과 조작부를 세로로 배치합니다.
