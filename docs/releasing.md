...
- 정식 태그는 `v0.1.1` 형태입니다. main 배포는 기존 태그와 초안 포함 릴리스보다 높은 다음 패치 버전을 선택합니다. Info.plist 버전은 최소 버전으로 사용합니다.
- 수동 실행의 version 입력으로 다음 마이너/메이저 버전을 지정할 수 있습니다. 기존 버전 이하나 잘못된 형식은 거부합니다.
- 앱 내부 빌드 번호는 `100 + GitHub run number`로 증가합니다. 태그를 이동하거나 기존 릴리스 파일을 덮어쓰지 않습니다.
