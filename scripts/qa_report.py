...
def main() -> None:
    ...
    </style><header><p>EXANOTE / 실제 화면 조작 기록</p><h1>사용자 관점 QA</h1><p>SUMMARY</p><p>기존 QA 문제의 수정 결과와 후속 화면 설정 검증을 아래에 기록했습니다. 캘린더는 후속 권한 허용 후 앱 재실행으로 연결됨을 확인했습니다. 실제 일정 조회와 재시작 없는 권한 갱신은 미검증입니다. Python 테스트 20개 통과, macOS 앱 빌드 성공. 로컬 QA 빌드 기준이며 배포 완료를 뜻하지 않습니다.</p><p>개별 테스트는 서브에이전트 한 개씩 순차 실행. PASS는 기록된 조작 범위에만 적용되며, BLOCKED와 미실행은 검증 완료를 뜻하지 않습니다.</p><button onclick="document.querySelectorAll('details').forEach(d=>d.open=true)">모든 증거 펼치기</button></header><h2>문제와 수정 결과</h2>ISSUES<h2>시나리오별 실행 기록</h2>SECTIONS</html>'''
